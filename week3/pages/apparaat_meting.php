...
if (isset($_GET['id'])) {
    $result = $mysqli->query("SELECT naam FROM apparaat WHERE id=". $_GET['id']) or die($mysqli->error);
    while ($row = $result->fetch_assoc()) {
        echo "<legend>" . $row['naam'] . "</legend>";
    }
    ...
    <script>
        ...
        function GetGraphic(comparison_type, region_type) {
            ...
            $.get("/inc/device-information.php?device_id=" + <?php echo $_GET['id'] ?> + "&comparison_type=" + comparison_type + "&region_type=" + region_type, function(data) {
                var load = <?php if (isset($_SESSION['huishouden_id'])) echo "true";
    else echo "false"; ?>;
                console.log(load);
                if (data != "" && load == true) {
